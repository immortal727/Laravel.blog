        ...
        <li class="">
            ...
            <div class="uk-navbar-dropdown uk-border-rounded uk-background-muted">
                <ul class="uk-nav uk-navbar-dropdown-nav">
                    @include('layouts._menu_categories', ['categories' => $category->children, 'is_child' => true])
                </ul>
            </div>
        </li>
    @else
        ...
        @isset($is_child)
            <li class="uk-nav-header"><a class="" href="{{ url($category->slug) }}">{{ $category->title ?? '' }}</a></li>
            @continue
        @endisset

        <li class=""><a href="{{ url($category->slug) }}">{{ $category->title ?? '' }}</a></li>
    @endif
@endforeach
